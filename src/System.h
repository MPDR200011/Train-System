 ...
 * Is responsible for handling queries and outputing information that is
 * requested. Also handles ticket purchasing.
 *
 */
class System {
public:
     ...
     *
     * Since the listings are handled using pointers, the destructor is in charge
     * od freeing the space occupied by them.
     *
     */
    ~System();
     ...
     *
     * Returns the pointer to the passenger object with id specified in the
     * parameter. If there is no such object, throws NoSuchPassengerException.
     *
     * @param id
     * @return Passenger*
     */
     ...
     *
     * Returns the pointer to the station object with id specified in the
     * parameter. If there is no such object, throws NoSuchStationxception.
     *
     * @param id
     * @return Station*
     */
     ...
     *
     * Creates a vector of trip that match the constraints given in the
     * parameters.
     *
     * @param src Pointer to source station object, if it is a nullptr, the
     * function will ignore this constraint.
     * @param dest Pointer to destination station object, if it is a nullptr, the
     * function will ignore this constraint.
     * @param searchByArrivalDate Bool checking wether to restrict the dates to
     * before the date specified.
     * @param arrivalDate Maximum arrival time desired. It's only considered if
     * searchByArrivalDate is set to true.
     * @return std::vector<Trip*>
     */
    std::vector<Trip *> searchTrips(Station *src,
                                    Station *dest,
     ...
     *
     * Sets the passanger with specified ID as inactive, effectively marking it as
     * removed.
     *
     * @param id
     */
     ...
     *
     * Sets the trip with specified ID as inactive, effectively marking it as
     * removed.
     *
     * @param id
     */
     ...
     *
     * Sets the station with specified ID as inactive, effectively marking it as
     * removed.
     *
     * @param id
     */
     ...
     *
     * Sets the train with specified ID as inactive, effectively marking it as
     * removed.
     *
     * @param id
     */
     ...
     *
     * Sets the engineer with specified ID as inactive, effectively marking it as
     * removed.
     *
     * @param id
     */
     ...
     *
     * Sets the repair shop with specified ID as inactive, effectively marking it
     * as removed.
     *
     * @param id
     */
     ...
     * @param arrivalDate
     */
    void createTrip(uint basePrice,
                    Station *source,
                    Station *destination,
                    Train *train,
                    Engineer *engy,
    ...
    /**
     * Finds the inactive engineer object with specified ID, and marks it as
     * active. Effectively hiring him.
     *
     * If there is no inactive engineer with the specified ID, throws
     * NoSuchEngineerException.
     *
     * @param id
     */
     ...
     * a specified distance from the provided station. And puts the specified
     * train object there, marking it as unhealthy.
     *
     * @param tr
     * @param st
     * @param distance
     ...
     *
     * Checks wether the trip has free seats and, if it does, adds the respective
     * trip to the passenger's vector of booked trips. Also calculates the price
     * to pay based on available discounts.
     *
     * @param request
     * @return true
     * @return false
     ...
     *
     * Works as a complement of the Train system abstraction, it's purpose is to
     * emulate an end of the month card processing: The System class has a vector
     * with passenger object pointers that specifies the passengers that have
     * cards and have not yet payed the monthly fee. "At the end of the month",
     * this method scans through the vector, removing the card pointer of any
     * passenger object that is still in it, signifying that it was invalidated
     * due to not being payed, and repopulates with the passengers that still have
     * cards.
     *
     */
    void processCards();
     ...
     * Looks for passenger object of id specified in the arguments, and removes
     * it's pointer from the cardsToPay vector.
     *
     * @param passengerID
...
private:
     ...
     */
    std::unordered_set<Engineer *,
            Engineer::EngineerHashUtils,
            Engineer::EngineerHashUtils>
            engineers;

    /**
     * Repair shops priority queue, keeps the ones that are more likely to become
     * available at top.
     */
    std::priority_queue<RepairShop *,
            vector<RepairShop *>,
            RepairShop::RepairShopPQUtils>
            shopsPQ;
    /**
