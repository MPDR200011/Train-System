...
vector<Passenger*> System::getPassengers() {
  ...
  for_each(passengers.begin(), passengers.end(),
           [&res](const pair<id_t, Passenger*>& item) {
...
vector<Station*> System::getStations() {
  ...
  for_each(stations.begin(), stations.end(),
           [&res](const pair<id_t, Station*>& item) {
             if (item.second->isActive()) {
               res.push_back(item.second);
             }
           });
...
vector<Train*> System::getTrains() {
  vector<Train*> res;
  for_each(trains.begin(), trains.end(),
           [&res](const pair<id_t, Train*>& item) {
             if (item.second->isActive()) {
               res.push_back(item.second);
             }
           });
...
vector<Trip*> System::searchTrips(Station* src,
                                  Station* dest,
...
void System::createStation(string& name, int x, int y) {
...
void System::createTrip(uint basePrice,
                        Station* source,
                        Station* destination,
                        Train* train,
                        Engineer* engy,
  ...
  Trip* tr = new Trip(basePrice, source, destination, train, engy,
...
void System::removeStation(id_t id) {
  ...
  for_each(tripIds.begin(), tripIds.end(),
           [this](id_t id) { trips[id]->setInactive(); });
...
bool System::payCard(id_t passengerID, ostream& os) {
  ...
  if (it != cardsToPay.end()) {
    ...
    os << "Card Price: " << costString << " euros" << endl;
    return true;
  } else {
    return false;
  }
...
void System::listPassengers(ostream& os) {
  ...
  sort(vec.begin(), vec.end(),
       [](const Passenger* p1, const Passenger* p2) -> bool {
         return p1->getName() < p2->getName();
       });
  os << endl
     << setw(5) << "id" << setw(35) << "name" << setw(15) << "birthdate"
...
void System::listHealthyTrains(ostream& os) {
  os << endl
     << setw(5) << "id" << setw(12) << "max seats" << setw(8) << "type" << endl;
...
void System::savePassengers() {
  ...
  for (auto& item : passengers) {
    ...
    passengersFile << p->getID() << " " << p->isActive() << " \""
                   << p->getName() << "\" \""
                   << p->getBirthDate().getDateString() << "\"" << endl;
...
void System::saveTrains() {
  ...
  for (auto& item : trains) {
    ...
    trainsFile << tr->getID() << " " << tr->isActive() << " "
               << tr->getMaxSeats() << " " << tr->getType() << " "
               << tr->isHealthy() << endl;
  }
  trainsFile.close();
}
...
void System::saveTrips() {
  ...
  for (auto& item : trips) {
    Trip* tr = item.second;
    tripsFile << tr->getID() << " " << tr->isActive() << " "
              << tr->getBasePrice() << " " << tr->getSource()->getID() << " "
              << tr->getDest()->getID() << " " << tr->getTrain()->getID() << " "
...
void System::savePurchases() {
  ...
  for (PurchaseLog& pl : sales) {
    purchasesFile << pl.getTripID() << " " << pl.getPassengerID() << " "
                  << pl.getPrice() << endl;
  }
  purchasesFile.clear();
}
  ...
  if (tripsFile.is_open()) {
    while (getline(tripsFile, line)) {
      ...
      try {
        ...
        Trip* trip =
            new Trip((uint)stoul(arguments[2]), src, dest, tr, eng, dep, arr);
  ...
  if (purchasesFile.is_open()) {
    while (getline(purchasesFile, line)) {
      ...
      try {
        ...
        PurchaseLog log((id_t)stoul(arguments[0]), p->getID(),
                        (uint)stoul(arguments[2]));
        p->logTrip(log);
        logPurchase(log);
      } catch (...) {
        continue;
...
void System::saveEngineers() {
  ...
  for (Engineer* eng : engineers) {
    engysFile << eng->getID() << " " << eng->isActive() << " "
              << "\"" << eng->getName() << "\" "
...
void System::createRepairShop(std::string& name, int x, int y) {
  RepairShop* rp = new RepairShop(name, x, y);
  srand((uint)time(nullptr));
  id_t id = rand();
  auto result = repairShops.insert(pair<id_t, RepairShop*>(id, rp));
  while (!result.second) {
    id = rand();
    result = repairShops.insert(pair<id_t, RepairShop*>(id, rp));
  }
  rp->setID(id);
  shopsPQ.push(rp);
}

void System::advanceDay() {
  vector<RepairShop*> rps;
  while (!shopsPQ.empty()) {
    rps.push_back(shopsPQ.top());
    shopsPQ.pop();
  }
  for (RepairShop* rp : rps) {
    rp->advanceDay();
    shopsPQ.push(rp);
  }
}

void System::saveRepairShops() {
  ofstream repairShopsFile("repair_shops.txt");
  string line;
  for (auto& item : repairShops) {
    item.second->save(repairShopsFile);
    repairShopsFile << endl;
  }
}

void System::loadRepairShops() {
  ifstream repairShopsFile("repair_shops.txt");
  if (repairShopsFile.is_open()) {
    string line;
    while (getline(repairShopsFile, line)) {
      vector<string> arguments = project_utils::splitArguments(line);
      if (arguments.size() < 6) {
        continue;
      }
      try {
        ...
        id_t shopID = (id_t)stoul(arguments[0]);
        int daysToAvailability = stoi(arguments[5]);
        int xCoord = stoi(arguments[3]);
        int yCoord = stoi(arguments[4]);
        RepairShop* rp = new RepairShop(name, xCoord, yCoord);
        if (arguments.size() > 6) {
          for (size_t i = 6; i < arguments.size(); i++) {
            Train* tr = getTrain(stoi(arguments[i]));
            rp->addTrain(tr);
          }
        }
        if (!stoi(arguments[1])) {
        ...
        rp->setID(shopID);
        rp->setDaysToAvailability(daysToAvailability);
        repairShops.insert(pair<id_t, RepairShop*>(shopID, rp));
        shopsPQ.push(rp);
      } catch (...) {
        continue;
      }
...
void System::sendTrainToRepairShop(Train* tr, Station* st, uint distance) {
  vector<RepairShop*> buff;
  RepairShop* shop = nullptr;
  while (!shopsPQ.empty()) {
    if (!shopsPQ.top()->isActive()) {
      buff.push_back(shopsPQ.top());
      shopsPQ.pop();
      continue;
    }
    if (project_utils::pointDistance(st->getX(), st->getY(),
                                     shopsPQ.top()->getXCoord(),
                                     shopsPQ.top()->getYCoord()) <= distance) {
      shop = shopsPQ.top();
      shopsPQ.pop();
      break;
    } else {
  ...
  if (shop != nullptr) {
    shop->addTrain(tr);
    shopsPQ.push(shop);
    cout << "Train was sent to repair shop: " << shop->getName() << "." << endl;
  }

  for (RepairShop* rp : buff) {
    shopsPQ.push(rp);
  }
...
void System::listRepairShops(std::ostream& os) {
  os << setw(14) << "ID" << setw(30) << "name" << setw(8) << "X" << setw(8)
     << "Y" << setw(15) << "available in" << endl;
  for (auto& item : repairShops) {
    if (item.second->isActive()) {
...
void System::removeRepairShop(id_t id) {
  auto it = repairShops.find(id);
  if (it != repairShops.end()) {
