...
void System::saveStations() {
    ...
    for (auto &item: stations) {
        ...
        stationsFile << st->getID() << " " << st->isActive() << " \"" << st->getName() << "\" "
                     << st->getX() << " " << st->getY() << endl;
    }
    stationsFile.close();
}
...
void System::saveTrains() {
    ...
    for (auto &item: trains) {
        ...
        trainsFile << tr->getID() << " " << tr->isActive() << " " << tr->getMaxSeats() << " " << tr->getType() << " "
                   << tr->isHealthy() << endl;
    }
    trainsFile.close();
}
...
void System::loadStations() {
    ...
    if (stationsFile.is_open()) {
        while (getline(stationsFile, line)) {
            ...
            try {
                Station *st = new Station(arguments[2], stoi(arguments[3]), stoi(arguments[4]));
...
void System::sendTrainToRepairShop(Train *tr, Station *st, uint distance) {
    ...
    while (!shopsPQ.empty()) {
        ...
        if (project_utils::pointDistance(st->getX(),st->getY(),shopsPQ.top()->getXCoord(),shopsPQ.top()->getYCoord()) <= distance) {
            shop = shopsPQ.top();
            shopsPQ.pop();
            break;
        } else {
...
void System::listRepairShops(std::ostream &os) {
    os << setw(14) << "ID" << setw(30) << "name"
    << setw(8) << "X" << setw(8) << "Y" << setw(15) << "available in" << endl;
    for (auto &item: repairShops) {
        if (item.second->isActive()) {
